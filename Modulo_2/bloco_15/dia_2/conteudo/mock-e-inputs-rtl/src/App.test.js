...
it('fetch joke', async () => { //É importante termos o async em it('fetch joke', async () => { , para que se possa utilizar await findByText onde estamos dizendo ao nosso teste: ei espere até que consiga encontrar esse texto no dom ou de erro por limite de tempo ;
  ...
  jest.spyOn(global, "fetch") //O jest.spyon espiona as chamadas a função fetch do objeto global , 
  global.fetch.mockResolvedValue({ //é por meio deste objeto global que conseguimos usar qualquer
    json: jest.fn().mockResolvedValue(joke),//função do sistema, por exemplo a função parseInt ;
  }); 
